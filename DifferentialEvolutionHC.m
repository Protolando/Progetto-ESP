function [y] = DifferentialEvolutionHC(f, ndim, populationSize, niter, a, b)
%Initialization
x=linspace(a,b,populationSize);
x=repmat(x,ndim,1);
...
for i = 1:niter
    ...
    nextGen = zeros(ndim, populationSize);
    ...
    %Only keep chromosomes bigger then their old generation counterpart
    for j = 1:populationSize
        y1 = f(x(:, j));
        y2 = f(nextGen(:, j));
        if(y1 > y2)
...
[~, i] = max(f(x));
%apply HillClimb
x = HillClimb(f, x(:, i), ndim);
%Check for problem bounds
for i = 1:ndim
    if(x(i) < a)
        x(i) = a;
    elseif(x(i) > b)
